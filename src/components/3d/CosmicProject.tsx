...
export const CosmicProject: React.FC<CosmicProjectProps> = ({
  ...
  return (
    <group ref={groupRef}>
      ...
      {(hovered || isSelected) && (
        <Text
          position={[0, projectConfig.size + 1.5, 0]}
          fontSize={0.3}
          color={projectConfig.color}
          anchorX="center"
          anchorY="middle"
      ...
      {/* Detailed project info on selection */}
      {isSelected && (
        <Html distanceFactor={20} position={[0, -projectConfig.size - 2, 0]}>
          <motion.div
            className="bg-gray-900/95 backdrop-blur-sm text-white p-6 rounded-2xl border-2 border-emerald-400/50 max-w-sm pointer-events-none shadow-2xl"
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {project.technologies.slice(0, 3).map((tech) => (
                <span
                  key={tech}
                  className="px-2 py-1 bg-emerald-500/20 text-emerald-300 text-xs rounded-full border border-emerald-400/30"
                >
                  {tech}
                </span>
              ))}
            ...
            <div className="text-xs text-gray-400">
              Click to explore this cosmic project
            </div>
          </motion.div>
        </Html>
      )}
